...
<form class="grid gap-4 sm:grid-cols-2" method="post" action="{{ route('admin.submissions.update',$business) }}">@csrf
    ...
    <x-city-select :cities="$cities" :value="$business->city" id="city-{{ $business->id }}" name="city" label="شهر" required /><label>آدرس<input class="field" name="address" value="{{ $business->address }}"></label>
