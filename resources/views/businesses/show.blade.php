...
<div class="grid gap-8 lg:grid-cols-[1fr_320px]">
    ...
    <aside><section class="panel lg:sticky lg:top-5"><h2 class="mb-5 text-xl font-bold">آدرس و اطلاعات تماس</h2><p class="leading-8">{{ $business->city }}، {{ $business->address }}</p>@if($business->phone)<p class="mt-4" dir="ltr">{{ $business->phone }}</p>@endif @if($business->website)<a class="mt-4 block break-all text-pomegranate" href="{{ $business->website }}" rel="nofollow noopener" target="_blank">وب‌سایت مکان ↗</a>@endif @if($business->opening_hours)<h3 class="mt-6 font-bold">ساعت کار به وقت تهران</h3><p class="mt-2 whitespace-pre-wrap text-sm leading-7">{{ $business->opening_hours }}</p>@endif</section></aside>
</div>
@endsection
